...
function fixDatepickerFloatingLabel(sender, args) {
	var txt = sender.get_id();
	var wrapper = sender.get_wrapperElement();
	var label = sender.get_element().parentElement.parentElement.nextElementSibling;
	wrapper.appendChild(label);
}
...
function AlternateControlVisibility(controlName, isContainer = false)
{
	try
	{
		var style = getComputedStyle(document.getElementById(controlName));
		if (isContainer)
		{
			if (style.display != "" && style.display != "flex" && style.display != "inline-flex")
			{
				document.getElementById(controlName).style.display = "flex";
			}
			else
		...
		{
			if (style.display != "" && style.display != "block" && style.display != "inline-block")
			{
				document.getElementById(controlName).style.display = "block";
			}
			else
