...
namespace PROJETO
{
	...
	public class MembershipUserProvider : IGeneralDataProvider
	{
		...
		private bool CheckUser(string UserName, string Password)
		{
			...
			CurrentItem.Fields["Login"].SetValue(UserName);
			CurrentItem = DataProvider.SelectItem(0, FormPositioningEnum.First) as LoginUserItem;
			if (CurrentItem != null)
			{
				return CurrentItem.Fields["Password"].Value.Equals(Password);
			}
			return false;
		}
		...
		public bool ChangePW(string UserName, string Password)
		{
			...
			CurrentItem.Fields["Login"].SetValue(UserName);
			CurrentItem = DataProvider.SelectItem(0, FormPositioningEnum.First) as LoginUserItem;
			CurrentItem.Fields["Password"].SetValue(Password);
	...
	public class GMembershipProvider : MembershipProvider
	{
		...
		{
			get { MembershipPasswordFormat passwordFormat = default(MembershipPasswordFormat); return passwordFormat; }
		}
