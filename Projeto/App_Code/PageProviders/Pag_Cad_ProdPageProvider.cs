...
namespace PROJETO.DataProviders
{
	...
	/// </summary>
	public class Pag_Cad_ProdPageProvider : GeneralProvider
	{

		public Pag_Cad_ProdPageProvider(IGeneralDataProvider Provider)
		{
			MainProvider = Provider;
			MainProvider.DataProvider = new DbCupcake_TB_PRODUTODataProvider(MainProvider, MainProvider.TableName, MainProvider.DatabaseName, "PK_TB_PRODUTO", "Pag_Cad_Prod");
		...
		public override bool Validate(GeneralDataProviderItem ProviderItem)
		{
			...
			try
			{
				Accepted =(ServerValidation.CheckNotEmpty(AliasVariables["NOME_PRODUTOField"]));
			}
			catch (Exception)
			{
				Accepted = false;
			}
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:RadTextBox_NOME_PRODUTO", "NOME_PRODUTO não pode ser vazio!");}
			try
			{
				Accepted =(ServerValidation.CheckNotEmpty(AliasVariables["VALOR_PRODUTOField"]));
			}
			catch (Exception)
			{
				Accepted = false;
			}
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:RadTextBox_VALOR_PRODUTO", "VALOR_PRODUTO não pode ser vazio!");}
			try
			{
				Accepted =(ServerValidation.CheckNotEmpty(AliasVariables["OBS_PRODUTOField"]));
			}
			catch (Exception)
			{
				Accepted = false;
			}
			if (!Accepted) { ProviderItem.Errors.Add("ServerValidationError:RadTextBox_OBS_PRODUTO", "OBS_PRODUTO não pode ser vazio!");}
