...
namespace PROJETO.DataProviders
{
	...
	public abstract class GeneralDataProvider : RecordDataProviderBase
	{
		...
		{
            Parameters.Clear();
            ((TableCommand)Select).WhereTemplate = new string[] { };
        }
		...
		private void InitializeDataProvider(IGeneralDataProvider BasePage, string TableName, string DataBaseName, string IndexName, string SqlFrom, bool IsVirtual)
		{
			...
			{
				IsView = true;
				if (SqlFrom.IndexOf("SQL_Select") == -1 &&
					SqlFrom.IndexOf("SQL_Where") == -1 &&
					SqlFrom.IndexOf("SQL_OrderBy") == -1)
				{
		...
		{
			FindRecord(Params, true);
		}

        public void FindRecord(Dictionary<string, object> Params, bool ShouldExecuteCount)
		{
			Item = BasePage.GetDataProviderItem(this);
			...
			else
			{
				...
				if (PageCount > 0 || !ShouldExecuteCount)
				{
					var ds = ExecuteSelect(0, 1);
					if (ds != null) dt = ds.Tables[0];
					if (!ShouldExecuteCount) this.PageNumber = 1;
				}
			}
		...
		private void SetOrderBy()
        {
			...
			if (string.IsNullOrEmpty(Select.OrderBy) && Select.Parameters.Count > 0)
            {
				string order = "";
                foreach (var parameter in this.Parameters)
                {
                    order += Dao.PoeColchetes(this.TableName, false) + "." + Dao.PoeColchetes(parameter.Value.Name, false) + ",";
                }
				Select.OrderBy = order.TrimEnd(',');
            }
            if (string.IsNullOrEmpty(Select.OrderBy) && Item.Fields.Count > 0)
			{
                Select.OrderBy = Dao.PoeColchetes(this.TableName, false) + "." + Dao.PoeColchetes(Item.Fields.First().Value.Name, false);
            }
        }
		...
		public DataSet SelectAllItems(GeneralDataProviderItem GeneralItem)
		{
			...
			{ 
				...
				dt = SelectJsonItem(new Dictionary<string, object>());
		...
		public int InsertItem(GeneralDataProviderItem Item)
		{
			...
			try
			{
				...
				if (PageProvider != null)
				{
					PageProvider.CreateEntries(EntryCommand.Insert);
					do
					{
						PageProvider.GetTableIdentity();
						BasePage.SetOldParameters(this.Item);
						string ValueField = "";
						foreach (Entry Entry in PageProvider.Entries.Values.Where(p => p.Position == i))
						{
							for (int vgNparc = 1; vgNparc <= Entry.NumberEntries; vgNparc++)
							{
								PageProvider.CreateEntriesItems(Entry, vgNparc);
								if (Entry.EntryItems != null && Entry.EntryItems.Count > 0)
								{
									if (this.Item.IntLan != null && Entry.CodLanField != null)
									{
										ValueField = this.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + this.Item[this.Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
										EntryItem CodLan = new EntryItem(this.Dao.PoeColAspas(Entry.Table), this.Dao.PoeColAspas(Entry.CodLanField), ValueField);
										Entry.EntryItems.Add(CodLan);
									}
						...
						if (PageProvider.Entries.Values.Count(p => p.Position == i) == 0) PageProvider.FillAuxiliarTables();
		...
		public void PrepareSelectCountCommands()
		{
			...
			{
                Select = new TableCommand(SqlFrom, GetParametersNames(ParameterType.Select), Dao);
            }
			Count = new TableCommand("SELECT COUNT(*) FROM " + SqlFrom, new string[] { }, Dao);
			if (!string.IsNullOrEmpty(SqlFrom) && SqlFrom.StartsWith("("))
			{
				Select.IgnoreFixCommand = true;
				Count.IgnoreFixCommand = true;
			}
		}
		...
		protected int DoUpdateItem()
		{
			...
			try
			{
				...
				if (PageProvider != null)
				{
					...
					// executa lancamentos na ordem	
					PageProvider.CreateEntries(EntryCommand.Update);
					int i = 0;
					do
					{
						string vgCodLan;
						if (Item.IntLan != null)
						{
							foreach (Entry Entry in PageProvider.Entries.Values.Where(p => p.Position == i))
							{
								...
								if (Entry.CodLanField != null && Entry.ExcludeEntry)
								{
									vgNparc++;
									vgCodLan = this.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + Item.Fields[Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
									while (Utility.PTab(Entry.Provider, Entry.CodLanField, vgCodLan))
									{
										vgNparc++;
										DataProcessEntry.ExecuteDeleteEntry(Entry.Table, vgCodLan, Update.Dao, Entry.CodLanField);
										vgCodLan = this.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + Item.Fields[Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
									}
								}
								
								for (vgNparc = 1; vgNparc <= Entry.NumberEntries; vgNparc++)
								{
									PageProvider.CreateEntriesItems(Entry, vgNparc);
									if (Entry.EntryItems != null && Entry.EntryItems.Count > 0)
									{
										vgCodLan = Entry.Provider.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + Item.Fields[Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
										List<EntryItem> LancList = new List<EntryItem>();
										foreach (EntryItem vgLanc in Entry.EntryItems)
										{
											LancList.Add(new EntryItem(vgLanc.TabelaAlvo, vgLanc.CampoAlvo, vgLanc.Valor, vgLanc.NumberEntryItems));
										}
										if (Entry.CodLanField != null && Utility.PTab(Entry.Provider, Entry.CodLanField, vgCodLan))
											DataProcessEntry.ExecuteUpdateEntry(LancList, vgCodLan, Entry.Provider.Dao, Entry.CodLanField);
										else
										{
											//incluir lan??amentos que nao estavam no insert original
											Entry EntryInclude = new Entry(Entry.Title, Entry.Table, Entry.NumberEntries, Entry.CodLanField, Entry.Provider);
											PageProvider.CreateEntriesItems(EntryInclude, vgNparc);
											if (Entry.CodLanField != null)
											{
												EntryItem CodLan = new EntryItem(Entry.Provider.Dao.PoeColAspas(Entry.Table), Entry.Provider.Dao.PoeColAspas(Entry.CodLanField), vgCodLan);
												EntryInclude.EntryItems.Add(CodLan);
											}
											DataProcessEntry.ExecuteInsertEntry(new List<EntryItem>(EntryInclude.EntryItems), Entry.Provider.Dao);
										}
									}
									else
									{
								...
								//Vamos excluir os lancamentos que ficaram sobrando
								if (Entry.CodLanField != null)
								{
									vgCodLan = this.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + Item.Fields[Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
									while (Utility.PTab(Entry.Provider, Entry.CodLanField, vgCodLan))
		...
		public int DoDeleteItem()
		{
			...
			try
			{
				...
				if (PageProvider != null)
				{
					...
					PageProvider.CreateEntries(EntryCommand.Delete);
					int i = 0;
					do
					{
						...
						if (Item.IntLan != null)
						{
							...
							foreach (Entry Entry in PageProvider.Entries.Values.Where(p => p.Position == i))
							{
								if (Entry.ExcludeEntry && Entry.CodLanField != null)
								{
									string vgCodLan;
									for (int vgNparc = 1; vgNparc <= Entry.NumberEntries; vgNparc++)
									{
										vgCodLan = this.Dao.ToSql(new TextField("", Entry.EntryControlKey + "-" + Item.Fields[Item.IntLan].GetFormattedValue("").PadLeft(8, '0') + Entry.EntryControlId + vgNparc.ToString().PadLeft(3, '0')).GetFormattedValue(""), FieldType.Text);
										DataProcessEntry.ExecuteDeleteEntry(Entry.Table, vgCodLan, Delete.Dao, Entry.CodLanField);
									}
								}
							}
						}
