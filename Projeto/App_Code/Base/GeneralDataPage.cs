...
namespace PROJETO.DataPages
{
	...
	public abstract class GeneralDataPage : Page, IGeneralDataProvider
	{
		...
		public DataAccessObject Dao
		{
			...
			{
				if (_dao == null)
					_dao = Utility.GetDAO(DatabaseName);
		...
		public int PageNumber
		{
			get 
			{
				...
				{
					return Convert.ToInt32(txtPageNumber.Split(',').Last());
				}
			}
		}
		...
		public virtual void ShowErrors()
		{
			...
			{
				string key = PageErrors.AllKeys[i];
				if (key.IndexOf("=") != -1)
				{
					string[] labels = key.Split(':')[1].Split('=')[1].Split(',');
					foreach (var label in labels)
					{
						this.FindControl(label).Visible = true;
					}
				}
				else
				{
		...
		protected virtual void ClearErrors()
		{
            ...
            foreach (var control in FindControlByAttribute(this, "IsValidationError", "true"))
            {
				control.Visible = false;
            }
		}

		public IEnumerable<Control> FindControlByAttribute(Control control, string key, string value)
		{
			var current = control as HtmlControl;
			if (current != null)
			{
            ...
            {
				var currentw = control as WebControl;
				if (currentw != null)
				{
					var k = currentw.Attributes[key];
					if (k != null && k == value)
					{
						yield return currentw;
					}
				}
			}
			if (control.HasControls())
			{
				...
				{
					foreach (Control item in FindControlByAttribute(c, key, value))
					{
						yield return item;
					}
				}
		...
		public void RegisterTelerikHiddenField(string name, string value)
		{
			if (AjaxPanel != null)
			{
				...
				{
					AjaxPanel.Controls.Add(new Literal() { Text = "", ID = name });
				}
		...
		private void PageLoad(object sender, System.EventArgs e)
		{
			...
			string filter = sWhere;

			if (!string.IsNullOrEmpty(filter))
			{
				if (string.IsNullOrEmpty(this.DataProvider.StartFilter))
				{
					this.DataProvider.StartFilter = filter;
				}
				else
				{
					this.DataProvider.StartFilter = "(" + this.DataProvider.StartFilter + ") AND " + "(" + filter + ")";
				}
			}
		...
		public virtual GeneralDataProviderItem PageSave()
		{
			...
			if(IsInsertig)
			{
				if(this.User.Identity.IsAuthenticated.ToString() == "False")
                {
					PageOperations.AllowInsert = CanInsert;
				}
				else
				{
					PageOperations.AllowInsert = CanInsert & Utility.CheckAllowAdd(this);
				}
			}
			else
			{
				if(this.User.Identity.IsAuthenticated.ToString() == "False")
                {
					PageOperations.AllowUpdate = CanEdit;
				}
				else
				{
					PageOperations.AllowUpdate = CanEdit & Utility.CheckAllowEdit(this);
				}
			}							
		...
		protected virtual int PageDelete()
		{
			...
			if (this.User.Identity.IsAuthenticated.ToString() == "False")
            {
                PageOperations.AllowDelete = CanDelete;
            }
            else
            {
                PageOperations.AllowDelete = CanDelete & Utility.CheckAllowRemove(this);
            }		
