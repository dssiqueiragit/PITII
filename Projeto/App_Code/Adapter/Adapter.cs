    ...
    public class Adapter
    {
        ...
        private string _QProcedures(string DatabaseName)
        {
            ...
            switch ((int)ServerType)
            {
                ...
                case 1://MYsql
                    return "(select ROUTINE_NAME as 'name', ROUTINE_DEFINITION as 'text',1 as 'colid', 'P' as 'xtype' from INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA = '" + DatabaseName + "' and ROUTINE_NAME != 'DROP_FOREIGNKEY') " +
                            "union " +
        ...
        ********************************************/
        ...
        private void LoadDBStruct(DataBaseStruct DBStruct, DataTable DT)
        {
            ...
            foreach (DataRow Row in DT.Rows)
            {
				...
				FieldST.IsNotForReplication = BoolParse(Row["is_identity"].ToString()) ? false : BoolParse(Row["is_not_for_replication"].ToString());
        ...
        *************************************************************************/
        ...
        {																																												//INT tem tamanho no MYSQL
            if (Field.FieldType.ToUpper() == "VARCHAR" || Field.FieldType.ToUpper() == "NCHAR" || Field.FieldType.ToUpper() == "VARBINARY" || Field.FieldType.ToUpper() == "NVARCHAR" || Field.FieldType.ToUpper() == "INT" || Field.FieldType.ToUpper() == "TINYINT" || Field.FieldType.ToUpper() == "NVARCHAR" && Field.FieldSize != null)
            {
        ...
        public string CreateFKCommand(string TableName, FKStruct FkStruct, bool IsCreate)
        {
            ...
            switch ((int)ServerType)
            {
                case 0: //SQL
                    ...
                    {
                        return ("IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_NAME='" + FkStruct.Name + "' AND TABLE_NAME='" + TableName + "') \r\n" +
                                         "BEGIN \r\n" +
        ...
        public string CreatePKCommand(string TableName, IndexStruct PkStruct, bool IsCreate)
        {
            ...
            switch ((int)ServerType)
            {
                case 0: //SQL
                    ...
                    if (IsCreate)
                    {
                        return ("IF not EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_NAME='" + PkStruct.Name + "' AND TABLE_NAME='" + TableName + "') \r\n" +
                                                                    "BEGIN \r\n" +
                                                                    "\tALTER TABLE [" + TableName + "] ADD  CONSTRAINT [" + ConstrantName + "] PRIMARY KEY " + IsCluster + "\r\n\t(" + ColumnName + ")" + FFactor + "\r\nEND");
                    }
                    else
                    {
                        return ("IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_NAME='" + ConstrantName + "' AND TABLE_NAME='" + TableName + "') \r\n" +
                                 "BEGIN \r\n" +
            ...
            foreach (ProcedureStruct NewProc in DBStructNew.Procedures)
            {
                ...
                if (ProcItemO != null)        // Encontrou a proc no DB velho
                {
                    ...
                    if (!NormalizeText(NewProc.CreateCommand.ToString()).Equals(NormalizeText(ProcItemO.CreateCommand)))
                    {
                        ...
                        Erro = true;
                        AddDiff("View e Procedures", "View/Procedure alterada: " + NewProc.Name);
                    }
                    else
                    {
                ...
                else
                {
                    NewProc.RunCreate = true;
                    Erro = true;
                    AddDiff("View e Procedures", "View/Procedure criada: " + NewProc.Name);
                }
            }
       ...
       *********************************************************************************************/
        ...
        {
            if (xtype.ToUpper() == "PROCEDURE" || xtype.ToUpper() == "FUNCTION")
            {
                switch (ServerType)
                {
                    case Util.DatabaseType.SQL:
        ...
        public string CreateScript(DataBaseStruct DB_Old, DataBaseStruct DB_New)
        {
            ...
            foreach (ProcedureStruct OldProc in DBStructOld.Procedures)                         //Vamos Dropar as procedures que sobraram 
            {
                ...
                if (OldProc.RunDrop)
                {
                    ...
                    AddDiff("View e Procedure", "View/Procedure deletada: " + OldProc.Name);
                }
            }
        ...
        public bool CompareFieldStruct(TableStruct TableStructOld, TableStruct TableStructNew)
        {
            ...
            foreach (FieldStruct FieldStructNew in TableStructNew.Fields)
            {
                ...
                if (FieldSTOld != null)                                               //Campo não existe vamos criar
                {
                    ...
                    if (!FieldStructNew.FieldType.Equals(FieldSTOld.FieldType)) 
                    {
                        if (!(FieldStructNew.FieldType.ToString() == "bigint" && (FieldSTOld.FieldType.ToString() == "int"))
                        && !(FieldStructNew.FieldType.ToString() == "int" && (FieldSTOld.FieldType.ToString() == "smallint")))
                        {
				FieldStructNew.ChangesFound += 4;
				FieldStructNew.Change = true;
                        }
                    }
                    if (!FieldStructNew.NumericPrecision.Equals(FieldSTOld.NumericPrecision) || !FieldStructNew.NumericScale.Equals(FieldSTOld.NumericScale))
                    {
                        if (!(FieldStructNew.FieldType.ToString() == "bigint" && (FieldSTOld.FieldType.ToString() == "int"))
                        && !(FieldStructNew.FieldType.ToString() == "int" && (FieldSTOld.FieldType.ToString() == "smallint")))
                        {
							FieldStructNew.ChangesFound += 8;
							FieldStructNew.Change = true;					
                        }
                    }
